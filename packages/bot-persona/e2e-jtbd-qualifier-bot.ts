	...
	viewDefinition: {
		nodes: [
			{ 
				id: "welcome", 
				components: [{
					message: { text: "👋 Привет! Я помогу вам понять, какой бот нужен именно вашему бизнесу. Поехали?" },
					buttonGroup: { buttons: [{ label: "Поехали!", event: "START" }] }
				}]
			},
			{ 
				id: "ask_name", 
				components: [{
					message: { text: "Для начала, как я могу к вам обращаться?" }
				}]
			},
			{ 
				id: "ask_job", 
				components: [{
					message: { text: "Какую основную задачу должен решать ваш бот? (можно выбрать несколько)" },
					buttonGroup: { buttons: [{ label: "Квалификация лидов" }, { label: "Поддержка (FAQ)" }, { label: "Далее", event: "SUBMIT_JOBS", payload: { selection: ["qualification", "support"] } }] }
				}]
			},
			{ 
				id: "ask_context", 
				components: [{
					message: { text: "А где именно бот будет работать?" },
					buttonGroup: { buttons: [{ label: "Telegram" }, { label: "Веб-сайт" }, { label: "Далее", event: "SUBMIT_CONTEXTS", payload: { selection: ["website"] } }] }
				}]
			},
			{ 
				id: "ask_struggles", 
				components: [{
					message: { text: "Понял. Какие узкие места в ваших процессах вы видите сейчас?" },
					buttonGroup: { buttons: [{ label: "Долго отвечаем на заявки", event: "SUBMIT_STRUGGLES", payload: { text: "Долго отвечаем на заявки" } }] }
				}]
			},
			{ 
				id: "ask_outcomes", 
				components: [{
					message: { text: "Ок. А достижение каких KPI будет для вас идеальным результатом?" },
					buttonGroup: { buttons: [{ label: "Рост конверсии", event: "SUBMIT_OUTCOMES", payload: { text: "Рост конверсии" } }] }
				}]
			},
			{ 
				id: "ask_extras", 
				components: [{
					message: { text: "Нужны ли какие-то дополнительные возможности?" },
					buttonGroup: { buttons: [{ label: "Интеграция с CRM" }, { label: "Сбор аналитики" }, { label: "Далее", event: "SUBMIT_EXTRAS", payload: { selection: ["crm", "analytics"] } }] }
				}]
			},
			{ 
				id: "ask_timeline", 
				components: [{
					message: { text: "Какие у вас желаемые сроки запуска?" },
					buttonGroup: { buttons: [{ label: "В течение месяца", event: "SELECT_TIMELINE", payload: { selection: "month" } }] }
				}]
			},
			{ 
				id: "ask_budget", 
				components: [{
					message: { text: "И последний вопрос, чтобы я мог предложить вам релевантное решение. В какой бюджет планируете уложиться?" },
					buttonGroup: { buttons: [{ label: "$1500+", event: "SELECT_BUDGET", payload: { selection: ">1500" } }] }
				}]
			},
			...
			{ 
				id: "pre_offer", 
				components: [{
					message: { text: "Спасибо, context.name! На основе ваших ответов, вам идеально подходит модель **context.recommendedModel**. Обычно такие проекты стоят **context.estimate.price** и занимают **context.estimate.timeline**. Это предварительная оценка." },
					buttonGroup: { buttons: [{ label: "Звучит интересно!", event: "CONTINUE" }, { label: "Спасибо, пока неактуально", event: "TOO_EXPENSIVE" }] }
				}]
			},
			...
			{ 
				id: "ask_show_examples", 
				components: [{
					message: { text: "Хотите, я покажу вам примеры подобных ботов в действии?" },
					buttonGroup: { buttons: [{ label: "Да, покажи", event: "ACCEPT_EXAMPLES" }, { label: "Нет, спасибо", event: "DECLINE_EXAMPLES" }] }
				}]
			},
			{ 
				id: "show_examples", 
				components: [{
					message: { text: "Отлично! Вот ссылка на релевантный кейс: [ссылка на кейс]. Изучите, и дайте мне знать, когда будете готовы продолжить." },
					buttonGroup: { buttons: [{ label: "Готов продолжить", event: "CONTINUE" }] }
				}]
			},
			{ 
				id: "book_call", 
				components: [{
					message: { text: "Супер! Финальный шаг — забронировать короткий звонок с нашим специалистом для обсуждения деталей. Вот ссылка на календарь: [ссылка на Calendly]" },
					buttonGroup: { buttons: [{ label: "Забронировал!", event: "BOOK_CALL" }] }
				}]
			},
			...
			{ 
				id: "nurturing_goodbye", 
				components: [{
					message: { text: "Понимаю. Спасибо, что уделили время! Чтобы вы могли лучше оценить потенциал, я буду иногда присылать вам полезные материалы по этой теме. Хорошего дня!" }
				}]
			},
			{ 
				id: "simple_goodbye", 
				components: [{
					message: { text: "Отлично! Рады будем поработать. Хорошего дня!" }
				}]
			},
		],
	},
