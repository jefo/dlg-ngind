import type { RenderedView, RenderViewInput } from "../../application";
...
export const renderViewAdapter = async (
	...
	if (Array.isArray(view.components)) {
		hydratedView.components = view.components.map((component: any) => {
			...
			if (
				hydratedComponent.props &&
				typeof hydratedComponent.props === "object"
			) {
				hydratedComponent.props = hydrateObject(
					hydratedComponent.props,
					context,
				);
			}
